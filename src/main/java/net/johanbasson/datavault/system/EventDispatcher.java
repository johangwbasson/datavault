...
public class EventDispatcher {
    ...
    public EventDispatcher(JmsTemplate jmsTemplate, String queue, ObjectMapper objectMapper) {
    ...
    private void sendToQueue(Event event) {
        try {
            log.debug("Sending {} to Rabbitmq", event.getClass().getName());
            retryTemplate.execute((RetryCallback<Void, Exception>) context -> {
                EventEnvelope envelope = new EventEnvelope(event.getClass(), objectMapper.writeValueAsString(event));
                jmsTemplate.convertAndSend(queue, objectMapper.writeValueAsBytes(envelope));
                return null;
            });
        } catch (Exception ex) {
            log.error("Could not send event to RabbitMq", ex);
        }
    }
}
